...
int sigma(double *d__, double *db, double *r__)
{
	/* System generated locals */
	double r__1;
...
/* Computing 2nd power */
	r__3 = ce * l[i__ - 1];
	dd = r__1 * r__1 + r__2 * r__2 + r__3 * r__3 + (h[i__ - 1] *
		k[i__ - 1] * ae * be * cce + k[i__ - 1] *
		l[i__ - 1] * be * ce * cae + l[i__ - 1] *
		h[i__ - 1] * ce * ae * cbe) * 2.0;

	if (dd < 0.0) dd = 0.0;
	d__ = 1.0 / sqrt(dd);
/* Computing 2nd power */
	r__1 = b[1];
	double t1 = 1.0 - r__1 * r__1 * dd;
	if (t1 < 0) t1 = 0;
	rad = sqrt(t1);
	f = div_number(b[1] * d__, rad);
	q[i__ - 1] = 1.0;
	q[i__ + 199] = div_number(1.0, (d__ * rad));
	q[i__ + 399] = f * h[i__ - 1] * (h[i__ - 1] * ae +	k[i__ - 1] * be * cce + l[i__ - 1] * ce * cbe);
	q[i__ + 599] = f * k[i__ - 1] * (k[i__ - 1] * be + l[i__ - 1] * ce * cae + h[i__ - 1] * ae * cce);
	q[i__ + 799] = f * l[i__ - 1] * (l[i__ - 1] * ce +	h[i__ - 1] * ae * cbe + k[i__ - 1] * be * cae);
	q[i__ + 999] = -f * k[i__ - 1] * l[i__ - 1] * be * ce *	sin(b[5]);
	q[i__ + 1199] = -f * l[i__ - 1] * h[i__ - 1] * ce *	ae * sin(b[6]);
	q[i__ + 1399] = -f * h[i__ - 1] * k[i__ - 1] * ae *	be * sin(b[7]);
/* L2: */
	t1 = div_number(b[1], d__);
	if (t1 > 1) t1 = 1;
	if (t1 < -1) t1 = -1;
	qq[i__ + npaf2 * 200 - 201] = b[0] + asin(t1);
	}
	i__1 = nr;
	for (ir = 1; ir <= i__1; ++ir) {
	j = 0;
	for (i__ = 1; i__ <= 8; ++i__) {
		...
		++j;
		qq[ir + j * 200 - 201] = q[ir + i__ * 200 - 201];
L3:
		;
	}
	}
...
int fonc(double *theta, double *r__, double *rr)
{
	/* System generated locals */
	int i__1;
	double r__1, r__2, r__3;
...
/* Computing 2nd power */
	...
	r2 += r__1 * r__1;
/* L2: */
	qq[i__ + npaf2 * 200 - 201] = yc;
	}
	*r__ = r1 / (nr - npaf);
	double t1 = div_number(r1, r2);
	if (t1 < 0) t1 = 0;
	*rr = sqrt(t1);
	return 0;
} /* fonc_ */
...
int celref(int *indi, double *bbb, double *afin, int *
	nhkl, double *theta, int *jhkl, double *ddt, double *ddq, FILE *imp_file)
{
...
/*      READ(5,*)NREP */
	...
	y2 = y0 / sin(qq[i + npaf2 * 200 - 201] - b[0]
		/ rd);
/* L367: */
		char temp[33];
...
FILE *createImpFile(char *file_name, const int procs)
{
	...
	{
		char *temp = trimwhitespace(buffer);
		if (temp[0] != '!' && temp[0] != '\0')
		{
			// fwrite(temp, nread, 1, inp_file);
			writeInFile(temp, inp_file);
			writeInFile("\n", inp_file);
		}
	}
...
int main(int argc, char *argv[])
{
    ...
    double diff2 = 0.0, diff, v1, del = 0.0, rmin, interest, tmax, ttmax, ncells, iiseed, ntried, ntriedt;
...
C*/
	...
	int i__3 = nruns2;
	for (int nrun2 = 1; nrun2 <= i__3; ++nrun2) {
...
/* ... Refine that cell */
			...
			dcell(celpre, al, &v1);
			calcul2(&diff, ihkl, th3, &ncalc, &igc);
			celref2(&indic, bb, afi, &lhkl, th3, ihkl, &ddt, &
				ddq);
			cncalc[igc - 1] = (double) ncalc;
			if (ndat >= 20) {
			fm20[igc - 1] = qo[19] / (cncalc[igc - 1] * 2.0 * ddq)
...
C*/
	...
	int i__1 = nruns2;
	for (int nrun2 = 1; nrun2 <= i__1; ++nrun2) {
...
/*     Which parameter to vary ? a or b or c or alpha or beta or gamma ? */
		...
		if (ip != 4 && ip != 5 && ip != 6) {
			...
			celpre[ip - 1] = astartt[ip - 4] + deltct[ip - 4] * 2.0 *
				randi(&iseed);
		}
...
L603:
		...
		if (ip2 != 4 && ip2 != 5 && ip2 != 6) {
			celpre[ip2 - 1] = pstartb[ip2 - 1] + del * (randi(&iseed)
				 - .5f) * 2.0;
		} else {
			celpre[ip2 - 1] = pstartb[ip2 - 1] + deld * (randi(&
				iseed) - .5f) * 2.0;
		}
		ang = celpre[3] + celpre[4] + celpre[5];
		if (ang >= 360.0 && ang <= 180.0) {
...
C*/
	...
	int end_parallel = 0;
	#pragma omp parallel default(shared) copyin(nhkl0, lhkl, ndat, dmin, slabda2, ihh, al, pi, cri, difp, difm, th2, fobs, sum_f, nind, w2, nmx, ndat10, ind, nhkl, indic) firstprivate (ncel,ntriedb, del, deld, v1, icode, llhkl, ihkl, th3, rmax2, a, v2, bpar, v3, pstartb, ipen, isee,  ip, x, diff, diff2, ncalc, ddt, ddq, iseed, iiseed, rmax0, ntried, ntriedt, nout, celpre, celold, rglob, nglob, rmin, rmax, bb, afi)
	{

		double part2 = pma[0] - pmi[0];
		double part = part2 / omp_get_num_threads();
		//part = (pma[0] - pmi[0]) / omp_get_num_threads();
		// part2 = part / pow(10, t);
		int id = omp_get_thread_num();

		double begin = pmi[0] + (part * id);
// printf("%d", end);
...
/* ...  here starts the loop */
	...
	#pragma omp parallel default(shared) copyin(nhkl0, lhkl, ndat, dmin, slabda2, ihh, al, pi, cri, difp, difm, th2, fobs, sum_f, nind, w2, nmx, ndat10, ind, nhkl, indic)  firstprivate (ncel,ntriedb, del, deld, v1, icode, llhkl, ihkl, th3, rmax2, a, c, v2, bpar, v3, pstartb, ipen, isee,  ip, x, diff, diff2, ncalc, ddt, ddq, iseed, iiseed, rmax0, ntried, ntriedt, nout, celpre, celold, rglob, nglob, rmin, rmax, bb, afi)
	{
...
/* ...  here starts the loop */
	...
	#pragma omp parallel default(shared) copyin(nhkl0, lhkl, ndat, dmin, slabda2, ihh, al, pi, cri, difp, difm, th2, fobs, sum_f, nind, w2, nmx, ndat10, ind, nhkl, indic) firstprivate (ncel,ntriedb, del, deld, v1, icode, llhkl, ihkl, th3, rmax2, a, b, c, v2, bpar, v3, pstartb, ipen, isee,  ip, x, diff, diff2, ncalc, ddt, ddq, iseed, iiseed, rmax0, ntried, ntriedt, nout, celpre, celold, rglob, nglob, rmin, rmax, bb, afi)
	{
...
/* ...  here starts the loop */
	...
	#pragma omp parallel default(shared) copyin(nhkl0, lhkl, ndat, dmin, slabda2, ihh, al, pi, cri, difp, difm, th2, fobs, sum_f, nind, w2, nmx, ndat10, ind, nhkl, indic) firstprivate (ncel,ntriedb, del, deld, v1, icode, llhkl, ihkl, th3, rmax2, a, b, c, bet, v2, bpar, v3, pstartb, ipen, isee,  ip, x, diff, diff2, ncalc, ddt, ddq, iseed, iiseed, rmax0, ntried, ntriedt, nout, celpre, celold, rglob, nglob, rmin, rmax, bb, afi)
	{
